class Ability
  ...
  def initialize(user)
    ...
    if user.admin?
      ...
      can [:manage, :switch], [AdminUser, Client, Team, TicketCategory, TicketStatus, TicketPriority, TicketType]
      ...
      can [:manage], [Ticket, TicketCategory, TicketStatus, TicketPriority, TicketType]
      
      can [:edit, :update, :change_password, :process_password_change, :profile], AdminUser, :id => user.id
      
      cannot :index, [AdminUser, Client, Team]
